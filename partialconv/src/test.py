...
class Tester(object):
    ...
    def iterate(self):
        ...
        if self.dataset_test_variance is not None:
            self.compute_statistics_model_based_cvar(self.model, self.config, self.test_writer, self.device, self.dataset_test_variance)
        else:
    ...
    def compute_statistics_cvar(self, model, config, writer, device, dataset):
        ...
        plt.savefig(self.config.ckpt + '/test/stats_cvar.pdf', bbox_inches="tight")

        plt.show()

    def compute_statistics_model_based_cvar(self, model, config, writer, device, dataset):
        model.eval()
